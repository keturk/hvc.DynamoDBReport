...
dynamo_query:
    REPORT report_name BEGIN_BLOCK 
    select_statement FROM table_name
    (index_statement | partition_statement)
    (WHERE where_statement (AND where_statement)*)?
    END_BLOCK;
    
report_name:     IDENTIFIER;

select_statement:
    SELECT output_column (COMMA output_column)*;
...
partition_statement: 
    WITH partition_key_name EQ partition_key_value (AND sortkey_statement)?;

partition_key_name: IDENTIFIER;
partition_key_value: string_with_quotes | parameter;

index_statement: 
    WITH index_name OPEN_PARA key_statement (AND sortkey_statement)? CLOSE_PARA;
...
between_sortkey_statement:    sortkey_name BETWEEN first_value AND second_value;
...
between_filter_statement:     filter_name BETWEEN first_value AND second_value;
exists_filter_statement:      filter_name (EXISTS | NOT EXISTS);
contains_filter_statement:    filter_name (CONTAINS | NOT CONTAINS) value;
