    ...
    public class DynamoDBReportGenerator : CodeGenerator
    {
        ...
        public String GenerateOutput(String fieldType, Criterion criterion)
        {
            switch (criterion)
            {
                case SimpleCriterion simpleCriterion:
                    return $"{fieldType}('{criterion.Name}').{ConvertCondition(simpleCriterion.Condition)}({ConvertValue(simpleCriterion.Value)})";
                case BetweenCriterion betweenCriterion:
                    return $"{fieldType}('{criterion.Name}').between({ConvertValue(betweenCriterion.FirstValue)}, {ConvertValue(betweenCriterion.SecondValue)})";
            ...
            _dynamoDBReport.Parameters.ForEach(parameter =>
            {
                _python
                    .Line(TemplateStack["ClickBody"]
                        .Add("param", parameter.Key.SnakeCase())
                        .Add("prompt", parameter.Key.CamelCase())
                        .Render());
        ...
        private void GenerateReportCommon()
        {
            _python
                ...
                .WriteToFile(Path.Combine(_outputFolder, "common", "report_common.py"));
        }
    }
}